...
boxplot(nc$weight~nc$habit,main="Effect of smoking on baby's weight",xlab="Habit",ylab="Baby's weight")
...
nc %>%
  group_by(mature) %>%
  summarise(min(mage),mean(mage),max(mage))
```
...
```{r pick-your-own}
# type your code for the Exercise here, and Knit
# Research question: Hypothesis test to check if average number of visits are different in the case of a premature birth or VS a full term birth
