...
at<-table(us12$response)[1]
nat<-table(us12$response)[2]
at
nat
```
...
str(us12)
df<-as.data.frame(table(us12$response))
head(df)
p=df[1,2]/sum(as.numeric(df$Freq))
se<-sqrt((p*(1-p))/sum(as.numeric(df$Freq)))
se
...
df2 <- atheism %>%
  filter(nationality %in% c("India","China") , atheism$year == "2012")

inference(y = response, data = df2, statistic = "proportion", type = "ci", method = "theoretical", success = "atheist")
```
...
inference(x = year, y = response, data = spain, statistic = "proportion", type = "ht", method = "theoretical", success = "atheist",null=0,alternative = "twosided")
```
...
inference(x = year, y = response, data = us, statistic = "proportion", type = "ht", method = "theoretical", success = "atheist",null=0,alternative = "twosided")
```
...
p=0.5 #as nothing is given about p
me=0.01
1.96^2*p*(1-p)/me^2
```
