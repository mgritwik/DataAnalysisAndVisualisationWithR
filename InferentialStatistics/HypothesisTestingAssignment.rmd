...
```{r set-seed}
set.seed(9102016)                 # make sure to change the seed
```
...
samp %>%
  summarise(sam_mean = mean(area), sam_med = median(area), 
            sam_sd = sd(area), sam_iqr = IQR(area),
...
samp_params
params
samp_params[[1]]<params[[1]]
samp_params[[2]]>params[[1]]
```
...
unique(ci_data$capture_mu)
t1<-table(ci[[4]])
(as.integer(t1["yes"])/(as.integer(t1["yes"])+as.integer(t1["no"])))*100
```
